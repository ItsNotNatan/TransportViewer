...
export default function FiltroOP({ atms, filtros, onFiltroChange, onLimpar, aberto, onClose }) {
  ...
  const opcoesFiltro = useMemo(() => {
    ...
    return {
      ids: idsOrdenados.map(id => ({ value: id, label: id })),
      ...
      <div className="modal-content fade-in" style={{ maxWidth: '600px', width: '100%', backgroundColor: 'white', borderRadius: '0.5rem', overflow: 'hidden', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
        ...
        <div className="modal-body" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem', overflow: 'visible' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
            ...
            {modoId === 'especifico' ? (
              <Select 
                isMulti options={opcoesFiltro.ids} value={currentEspecifico} onChange={handleEspecificoChange} 
                placeholder="Selecione um ou mais ATMs..." styles={selectStyles} noOptionsMessage={() => "Nenhum ATM encontrado"}
              />
            ) : (
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  <Select options={opcoesFiltro.ids} value={currentDe} onChange={(opt) => handleRangeChange('de', opt)} placeholder="ID Inicial (De)" styles={selectStyles} isClearable />
                </div>
                <span style={{ color: '#6b7280', fontWeight: 'bold' }}>até</span>
                <div style={{ flex: 1 }}>
                  <Select options={opcoesFiltro.ids} value={currentAte} onChange={(opt) => handleRangeChange('ate', opt)} placeholder="ID Final (Até)" styles={selectStyles} isClearable />
                </div>
              </div>
            )}
          </div>
